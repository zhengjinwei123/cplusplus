...
namespace aoi {
	...
	std::unordered_map<std::string, dtNavMesh*> CDetour::mStaticMesh;
	std::mutex CDetour::mMutex;
...
#pragma warning(pop)
	...
	CDetour::CDetour(bool bStaticMesh, uint16_t maxNode)
	...
	int CDetour::Load(const char*path) {
	...
	dtNavMesh* CDetour::createStaticMesh(const char*path, int& errCode) {
		dtNavMesh* mesh = nullptr;
		std::lock_guard<std::mutex> lock(mMutex);
		auto it = CDetour::mStaticMesh.find(path);
		if (it != CDetour::mStaticMesh.end()) {
			mesh = it->second;
		}
		else {
			mesh = loadStaticMesh(path, errCode);
			if (!errCode) {
				CDetour::mStaticMesh[path] = mesh;
			}
		}
		return mesh;
	}

	dtNavMeshQuery* CDetour::GetQuery() {
		return mQuery;
	}

	dtNavMesh* CDetour::loadStaticMesh(const char*path, int& errCode) {
		errCode = 0;
		FileReader fp(path);
		if (fp == 0) {
			errCode = 101;
	...
	dtNavMesh* CDetour::loadDynamicMesh(const char*path, int& errCode) {
		errCode = 0;
		FileReader fp(path);
		if (fp == 0) {
			errCode = 201;
		...
		mTalloc = new CLinearAllocator(32 * 1024);
		mTcomp = new CFastLZCompressor();
		mTmproc = new CMeshProcess();
	...
	bool CDetour::RandomPosition(float halfExtents[3],
		const dtQueryFilter* filter,
		float(*frand)(),
		uint64_t& randomRef,
		float randomPt[3])
	...
	unsigned int CDetour::AddCapsuleObstacle(const float pos[3], const float radius, const float height) {
		if (!mTileCache) {
			return 0;
		}
	...
	unsigned int CDetour::AddBoxObstacle(const float bmin[3], const float bmax[3]) {
		if (!mTileCache) {
			return 0;
		}
		dtObstacleRef obstacleId;
	...
	unsigned int CDetour::AddBoxObstacle(const float center[3], const float halfExtents[3], const float yRadians) {
		if (!mTileCache) {
			return 0;
		}
		dtObstacleRef obstacleId;
	...
	void CDetour::RemoveObstacle(unsigned int obstacleId) {
