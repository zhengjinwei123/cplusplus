...
void NavMeshTesterTool::recalc()
{
	...
	// add by zjw
	std::cout << "-----------A*Â·¾¶----start---------" << std::endl;
	for (int i = 0; i < m_nsmoothPath; ++i) {
		std::cout << "( " << m_smoothPath[i * 3] << "," << m_smoothPath[i * 3 + 1]  << "," << m_smoothPath[i * 3 + 2] << " )" << std::endl;
	}
	std::cout << "-----------A*Â·¾¶----end---------" << std::endl;
...
void NavMeshTesterTool::handleRender()
{
    ...
    if (m_toolMode == TOOLMODE_PATHFIND_FOLLOW)
    {
        ...
        {
            dd.depthMask(false);
            const unsigned int spathCol = duRGBA(255, 0, 0, 220);
