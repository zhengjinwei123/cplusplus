...
#define BIND_FREE_FUNC(_free_func_ptr) \
    _getFunctionFactory(_free_func_ptr).bind<_free_func_ptr>()

#define BIND_MEM_FUNC(_mem_func_ptr, _instance_ptr) \
    _getFunctionFactory(_mem_func_ptr).bind<_mem_func_ptr>(_instance_ptr)

#define BIND_TEMPLATE_MEM_FUNC(_mem_func_ptr, _instance_ptr) \
    _getFunctionFactory(_mem_func_ptr).template bind<_mem_func_ptr>(_instance_ptr)


template<typename FunctionSiganture>
class Function;
...
// param 0 test
typedef Function<void ()> MessageCb0;
void test0(const MessageCb0 &cb) {
    cb();
}

// param 1 test
typedef Function<void (int)> MessageCb1;
void test1(int a,const MessageCb1 &cb_func) {
    cb_func(a);
}
void test1func(int a) {
    cout<<"test1 called:"<<a<<endl;
}

// pram 2 test
typedef Function<void (int,int)> MessageCb2;
void test2(int a, int b, const MessageCb2 &cb) {
    cb(a, b);
}
...
public:
    void test() {
        cout << "cls2 test called"<< endl;
    }

    void test1(int a, int b) {
        cout<<"Cls2 test 1 called:"<<a << "," << b << endl;
    }
};
...
{
    Cls cls;
    test0(BIND_MEM_FUNC(&Cls::test, &cls));
    test1(11, BIND_FREE_FUNC(&test1func));
    test2(1,2, BIND_MEM_FUNC(&Cls::test1, &cls));
